...
public class Gitlet implements Serializable{
	...
	public void rebase(String branchName) {
		...
		Commit splitPoint = pointer2;
		for (String s: currentHead.snapShot.keySet()) {
			if (!splitPoint.snapShot.containsKey(s)) {
				differences.add(s);
			}
		}
		for (Commit c : commitCopies) { // re-commit commit copies
			...
			for (String s: differences) {
				File f = new File(".gitlet/commits/commit" + branchCommit.getID() + "/" + s);
				newCommit.snapShot.put(f.getPath(), f);
			}
			for (File f : stagingArea.values()){
				newCommit.snapShot.put(f.getPath(), f);
